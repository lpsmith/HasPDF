...
drawWords :: (Style s) => s -> [HBox s] -> Point -> Draw ()
drawWords _ [] _  = return ()
...
drawPureWords :: Style s => s -> [HBox s] -> Point -> PDFText ([HBox s],PDFFloat)
...
startDrawingNewLineOfText :: (Style s) => PDFFloat -> PDFFloat -> [HBox s] -> Point -> s -> Draw ()
...
drawLineOfHboxes :: (Style s) => PDFFloat -- ^ Height of the total line first time this function is called
                 ...
                 -> Point    -- ^ x for the remaining boxes, y for the whole line
                 -> Draw ()
drawLineOfHboxes _ _ [] _ = return ()
-- | Start a new text
drawLineOfHboxes hl dl l@((Text style _ _):_) xy = startDrawingNewLineOfText hl dl l xy style
drawLineOfHboxes hl dl l@((HGlue _ _ (Just style)):_) xy = startDrawingNewLineOfText hl dl l xy style
...
drawTheTextBox :: Style style => TextDrawingState
               ...
               -> Maybe PDFString
               -> PDFText ()
drawTheTextBox state style xy t = do
